...
class ManipulationMixin(MenloConnection):
    ...
    async def place(
        ...
        held_ids = (await self._robot_state_model()).robot.held_entity_ids
        if len(held_ids) != 1:
        ...
        held_after = RobotState.model_validate(
            response.get("robot_state", {})
        ).robot.held_entity_ids
        placement = {
        ...
        if held_id in held_after or placed is None:
            placement["status"] = "still_held" if held_id in held_after else "missing"
